...
async function countLines() {
  const targetDirs = ['.', './cmds', './events', './slash'];
...
module.exports = {
  ...
  async execute(message, args, client, db) {
    ...
    const titleText = new TextDisplayBuilder().setContent('# NextGen Helper Bot');

    const descriptionText = new TextDisplayBuilder().setContent(
      `> **Welcome to NextGen Helper!**\n\n` +
    ...
    const inviteButton = new ButtonBuilder()
      .setLabel('NextGen DashBoard')
      .setStyle(ButtonStyle.Link)
      .setURL('https://nextgen.kozow.com/');
